...
function cariMotorDariUserId(
  ...
  // setting halaman dan query
  $stmt_jml_motor = mysqli_query(
    $conn,
    "SELECT plat 
     FROM motor 
     WHERE id_user_pemilik = $id_user_pemilik AND (plat LIKE '%$keyword%' OR lokasi_parkir LIKE '%$keyword%')
    "
  );
  ...
  $stmt = mysqli_prepare(
    ...
    "SELECT plat, lokasi_parkir, tanggal_masuk, id_user_pemilik
     FROM motor
     WHERE id_user_pemilik = ? AND (plat LIKE ? OR lokasi_parkir LIKE ?)
     ORDER BY tanggal_masuk DESC
     LIMIT ?, ?
    "
  );
